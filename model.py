...
class EncDecDraft(nn.Module):
    def __init__(self, compact_dim, t_max, cfg: ModelConfig):
        ...
        enc = nn.TransformerEncoderLayer(d, cfg.n_heads, cfg.ff_dim, dropout=cfg.dropout, batch_first=True, activation="gelu", norm_first=True)
        self.encoder = nn.TransformerEncoder(enc, cfg.n_layers)
        dec = nn.TransformerDecoderLayer(d, cfg.n_heads, cfg.ff_dim, dropout=cfg.dropout, batch_first=True, activation="gelu", norm_first=True)
